...
internal static class StringExtensions
{
    ...
    private static readonly Regex ImportRegex = new("""import\s*\{[^}]*\}\s*from\s*["'](?<path>\./[^"']+)["']""",
        RegexOptions, TimeSpan.FromMilliseconds(200));

    private static readonly Regex ObjectRegex =
        new(@"const\s+\w+\s*=\s*(\{.*?\})\s*;", RegexOptions, TimeSpan.FromMilliseconds(200));

    private static readonly Regex JsObjectReplacerRegex =
        new(@"(?<=\{|,)\s*(\w+)\s*:", RegexOptions, TimeSpan.FromMilliseconds(200));
