...
/// https://github.com/Kareadita/Kavita/blob/develop/API/Services/Tasks/Scanner/Parser/Parser.cs on 2026/01/17.
/// Original Copyright (c) [2020] majora2007
/// Modifications Copyright (c) 2026 Amelia / Mnema
/// </summary>
///
...
public partial class ParserService: IParserService
{
    ...
    private static readonly ImmutableArray<string> FormatTagSpecialKeywords =
    [
    ...
    /// </summary>
    private const string BalancedBracket = @"(?:[^\[\]]|(?<open>\[)|(?<-open>\]))*?(?(open)(?!))";
    /// <summary>
    ...
    private static readonly Regex CssImageUrlRegex = new(@"(url\((?!data:).(?!data:))" + "(?<Filename>(?!data:)[^\"']*)" + @"(.\))",
        MatchOptions, RegexTimeout);

    /// <summary>
    /// An Appropriate guess at an ASIN being valid
    /// </summary>
    private static readonly Regex AsinRegex = new(@"^(B0|BT)[0-9A-Z]{8}$",
        MatchOptions, RegexTimeout);
    ...
    private static readonly Regex ArchiveFileRegex = new(ArchiveFileExtensions,
        MatchOptions, RegexTimeout);
    private static readonly Regex EpubFileRegex = new(EpubFileExtension,
        MatchOptions, RegexTimeout);
    ...
    public Format ParseFormat(string filePath)
    {
        if (IsArchive(filePath)) return Format.Archive;
        if (IsEpub(filePath)) return Format.Epub;

        return Format.Unsupported;
    }

    public string ParseEdition(string filePath)
    {
    ...
    /// <returns></returns>
    public bool HasSpecialMarker(string? filePath)
    {
        if (string.IsNullOrEmpty(filePath)) return false;
        return SpecialMarkerRegex.IsMatch(filePath);
    }

    public int ParseSpecialIndex(string filePath)
    {
    ...
    public bool IsSpecial(string? filePath)
    {
        return HasSpecialMarker(filePath);
    }

    private string ParseMangaSeries(string filename)
    {
    ...
    public string ParseComicSeries(string filename)
    {
    ...
    public string ParseMangaVolume(string filename)
    {
    ...
    public string ParseComicVolume(string filename)
    {
    ...
    private string FormatValue(string value, bool hasPart)
    {
        if (!value.Contains('-'))
        {
    ...
    private static string RemoveLeadingZeroes(string title)
    {
        var ret = title.TrimStart(LeadingZeroesTrimChars);
        return string.IsNullOrEmpty(ret) ? "0" : ret;
    }

    public Regex FileExtensionsForFormat(Format format)
    {
        return format switch
        {
            Format.Archive => ArchiveFileRegex,
            Format.Epub => EpubFileRegex,
            _ => throw new ArgumentOutOfRangeException(nameof(format), format, null)
        };
    }

    public string ParseSeries(string filename, ContentFormat type)
    {
        return type switch
        {
            ContentFormat.Manga => ParseMangaSeries(filename),
            ContentFormat.Comic => ParseComicSeries(filename),
            ContentFormat.Book or ContentFormat.LightNovel => ParseMangaSeries(filename),
            _ => string.Empty
        };
    }
    ...
    public string ParseVolume(string filename, ContentFormat type)
    {
        return type switch
        {
            ContentFormat.Manga => ParseMangaVolume(filename),
            ContentFormat.Comic => ParseComicVolume(filename),
            ContentFormat.Book or ContentFormat.LightNovel => ParseMangaVolume(filename),
            _ => LooseLeafVolume
        };
    }

    public string ParseChapter(string filename, ContentFormat type)
    {
        return type switch
        {
            ContentFormat.Manga => ParseMangaChapter(filename),
            ContentFormat.Comic => ParseComicChapter(filename),
            ContentFormat.Book or ContentFormat.LightNovel => ParseMangaChapter(filename),
            _ => DefaultChapter
        };
    }

    private string ParseMangaChapter(string filename)
    {
    ...
    private string AddChapterPart(string value)
    {
        if (value.Contains('.'))
        {
            return value;
    ...
    private string ParseComicChapter(string filename)
    {
    ...
    private string RemoveEditionTagHolders(string title)
    {
    ...
    public string CleanTitle(string title, bool isComic = false)
    {
    ...
    public bool IsComicInfoExtension(string filePath)
    {
        return ComicInfoArchiveRegex.IsMatch(Path.GetExtension(filePath));
    }
    public bool IsBook(string filePath)
    {
    ...
    public float MinNumberFromRange(string range)
    {
        try
        {
    ...
    public float MaxNumberFromRange(string range)
    {
        try
        {
            // Check if the range string is not null or empty
    ...
    public string CleanSpecialTitle(string? name)
    {
    ...
    /// <returns></returns>
    public bool IsCoverImage(string filename)
    {
    ...
    /// <returns></returns>
    public bool HasBlacklistedFolderInPath(string path)
    {
    ...
    public bool IsEpub(string filePath)
    {
    ...
    /// <returns></returns>
    public bool HasComicInfoSpecial(string comicInfoFormat)
    {
    ...
    /// <param name="name"></param>
    /// <returns></returns>
    public bool IsSeriesAndYear(string? name)
    {
    ...
    /// <param name="name"></param>
    /// <returns></returns>
    public string ParseYear(string? name)
    {
        if (string.IsNullOrEmpty(name)) return string.Empty;
    ...
    public string? RemoveExtensionIfSupported(string? filename)
    {
    ...
    /// <returns></returns>
    private string RemoveDuplicateVolumeIfExists(string filename)
    {
     ...
     */
    public string PrettifyFileName(string name)
    {
    ...
    public string RemoveDuplicateChapterIfExists(string filename)
    {
    ...
    /// <returns></returns>
    public bool IsLikelyValidAsin(string? asin)
    {
    ...
    public bool IsDefaultChapter(string? chapterNumber)
    {
